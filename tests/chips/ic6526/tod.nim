...
proc tenths_advance =
  ...
  for i in 0..5:
    for _ in 1..5:
      set traces[TOD]
      check (read_register TOD10TH) == uint8 i
      clear traces[TOD]
    set traces[TOD]
    check (read_register TOD10th) == uint8 (i + 1)
    clear traces[TOD]
...
proc seconds_advance =
  ...
  for t in 0..9:
    check (read_register TOD10TH) == uint8 t
...
proc minutes_advance =
  ...
  for s in 0..59:
    for t in 0..9:
      check:
        (read_register TOD10TH) == uint8 t
        (read_register TODSEC) == bcd uint8 s
      # this loop cycles the clock 6 times, advancing the tenths once
      for _ in 1..6:
        set traces[TOD]
        clear traces[TOD]
    ...
    for s in 0..59:
      for t in 0..9:
        check:
          (read_register TOD10TH) == uint8 t
          (read_register TODSEC) == bcd uint8 s
          (read_register TODMIN) == bcd uint8 m
        # this loop cycles the clock 6 times, advancing the tenths once
        for _ in 1..6:
          set traces[TOD]
          clear traces[TOD]
