...
# IRQ stuff can still be tested without access to the raster counter, so we redefine a
# a couple of tests to be usable in release mode
when defined(release):
  proc raster_irq =
    ...
    write_register(RASTER, 0x01)
    write_register(CTRL1, 0x80)
    while not lowp(traces[IRQ]): cycle(traces[PHIDOT])
  ...
  proc raster_irq_disabled =
    ...
    for _ in 0..262:
      check:
        trip(traces[IRQ])
        read_register(IR) == 0b01110000 # only unused bits are set
      for _ in 1..65: cycle(traces[PHIDOT])
